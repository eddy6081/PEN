...
#########################################################################################
#########################################################################################
def get_image_regions(pred_masks, pred_boundary, gt_masks, gt_boundary, mask_thresh=0.5, boundary_thresh=0.8, size_thresh=100):
	"""
	Parameters
	----------------
	pred_masks, pred_boundary: 2D or 3D array of floats
		each item is an array of predicted (range 0-1) mask / boundary of objects
		with size [H x W] or [H x W x N]
	gt_masks, gt_boundary: 2D or 3D boolean array
	...
	boundary_thresh: float
		threshold to apply to predicted boundaries
	size_thresh: float
	...
	if gt_masks.ndim<3:
		...
		gt_boundary = np.expand_dims(gt_boundary,axis=-1)
	if pred_masks.ndim<3:
		pred_masks = np.expand_dims(pred_masks,axis=-1)
		pred_boundary = np.expand_dims(pred_boundary,axis=-1)
	...
	for ch in range(gt_masks.shape[2]):
		gt_labels = gt_masks[:,:,ch]#np.logical_and(gt_masks[:,:,ch], np.logical_not(gt_boundary[:,:,ch])) #label makes it a tuple, remove tuple with [0]
		#changed 06/11/22 CE
		...
		#changed CE 06/11/22: with the removal of small objects can happen at this stage.
	...
	for ch in range(pred_masks.shape[2]):
		ch_pred_mask = pred_masks[:,:,ch]>=mask_thresh #changed CE 06/10/22
		#np.logical_and(pred_masks[:,:,ch]>=mask_thresh,np.logical_not(pred_boundary[:,:,ch]>=boundary_thresh))
		#the previous steps can make small objects that should not be labeled.
...
def get_matched_mask_qualities(all_matches, all_ious):
	...
	RETURNS
	----------------------------------------------------
	Q = np.array float vector with length of alpha IoU thresholds
	 	with quality of matched masks as average intersection over union
	"""
